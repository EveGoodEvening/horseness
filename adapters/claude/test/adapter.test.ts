...
test("Claude live receipt validates the required subscription auth mode and rejects auth/account fields", () => {
  ...
  const receipt: ClaudeSubscriptionLiveReceiptV1 = { schemaVersion: "ClaudeSubscriptionLiveReceiptV1", host: "claude", authMode: "existing-user-subscription-session", hostVersion: "2.1.228", observedModel: "claude-model", candidate: { head: "head", tree: "tree" }, command: { argv: ["pnpm", "host:smoke:claude"], digest, scenarioSetDigest: digest, batchResponseDigest: digest }, provenance: { archiveDigest: digest, archiveIdentity: "npm:claude", memberPath: "claude", executableDigest: digest, packageDigest: CLAUDE_NATIVE_PACKAGE_METADATA.packageDigest, contributions: CLAUDE_NATIVE_PACKAGE_METADATA.contributions.map(({ name, digest: contributionDigest }) => ({ name, digest: contributionDigest })) }, bindings: Array.from({ length: 5 }, (_, index) => ({ ...receiptBinding, attemptId: `a-${index}` })), redactionAudit: { passed: true, prohibitedFields: [] }, timing: { startedAt: "2026-01-01T00:00:00Z", finishedAt: "2026-01-01T00:00:01Z", durationMs: 1000 }, terminal: { result: "succeeded", reason: "CLAUDE_LIVE_SMOKE_SUCCEEDED" } };
  assert.deepEqual(validateClaudeSubscriptionLiveReceiptV1(receipt), receipt);
  assert.throws(() => validateClaudeSubscriptionLiveReceiptV1({ ...receipt, authMode: undefined } as unknown as ClaudeSubscriptionLiveReceiptV1), /INVALID/);
  assert.throws(() => validateClaudeSubscriptionLiveReceiptV1({ ...receipt, authMode: "api-key" } as unknown as ClaudeSubscriptionLiveReceiptV1), /INVALID/);
