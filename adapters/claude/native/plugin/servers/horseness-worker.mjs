...
const runtimeNonce = process.env.HORSENESS_CLAUDE_RUNTIME_NONCE;
...
function invokeRuntime(input) {
  return new Promise((resolve, reject) => {
    ...
    socket.once("connect", () => socket.end(`${JSON.stringify({ schemaVersion: "HorsenessClaudeRuntimeRequestV1", nonce: runtimeNonce, sessionId: process.env.CLAUDE_CODE_SESSION_ID ?? null, input })}\n`));
    ...
    socket.once("end", () => {
      ...
      try {
        ...
        if (parsed.schemaVersion !== "HorsenessClaudeRuntimeResponseV1" || parsed.ok !== true) throw new Error(typeof parsed.reason === "string" ? parsed.reason : "HORSENESS_NATIVE_RUNTIME_REJECTED");
