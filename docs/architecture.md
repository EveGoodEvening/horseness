...
The first release uses npm registry integrity, npm-generated provenance, repository source, and a protected GitHub `release` environment. The initial publication MAY use a short-lived package-scoped granular npm token because npm trusted publishers can be configured only after a package exists; the token is removed after the initial release. Subsequent releases SHOULD use npm trusted publishing from `.github/workflows/release.yml`. Static signing keys, custom KMS signers, project-root ceremonies, and custom immutable-storage credentials are outside the v1 release path.
