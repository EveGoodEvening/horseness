...
### Neutral signed install bundles and host roots

The installer core consumes only `NeutralInstallBundleV1`; it MUST NOT import an adapter package or execute adapter contribution code. The signed release manifest contains a closed, sorted contribution catalog with `adapterId`, `hostId`, supported `platform/arch` tuples, pinned host version, package/source-artifact digest, support classification, and each exact contribution's relative path, kind, mode, size, content digest, and bytes-member digest. The executable bootstrap/top edge verifies the release manifest and artifact/member bytes, constructs the neutral bundle from those authenticated records, and supplies both that bundle and a public daemon client to installer operations. Installer verification independently hashes every received contribution byte and rejects missing, extra, reordered, unsupported, or digest/mode/path-substituted records before mutation. Adapter metadata may be a release-build input, but adapters are never a runtime dependency of `packages/installer`.
