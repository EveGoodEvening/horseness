...
The first release targets Node.js 22, strict TypeScript, ESM, and a pnpm monorepo. It publishes the provider-neutral core, local daemon, CLI, transactional installer library, and Pi/OMP/Claude Code/Codex adapter packages to npm. SQLite in WAL mode and an append-only event log are authoritative. Large evidence and context artifacts are content-addressed files. Operation remains workspace-local by default. The self-contained bootstrap and offline distribution are deferred from the first public release.
...
Horseness stores only opaque keychain/host credential references. The secure process boundary uses argv arrays, realpath-constrained executable/cwd, allowlisted environment, credential removal by default, and bounded/redacted receipts. A controlled live-host gate may resolve specifically allowlisted opaque CI keychain references for least-privilege test accounts under network/time/cost limits; validator, fake-host, local deterministic-provider, and credentialed-live receipts are distinct. Claude Code and Codex are the explicit exception defined below: their live gates use the invoking OS user's existing native subscription session and require neither an opaque CI provider secret reference nor Horseness-managed provider-session rotation.

### Claude Code and Codex subscription-session authentication exception

The general opaque-reference and controlled-live credential rules remain normative for every other credential and host. By explicit user authorization, C17, C18, and their later exact-public verification MAY instead drive the already authenticated native Claude Code or Codex subscription session of the invoking OS user. This exception supplies provider authentication and billing only. Horseness workspace and attempt authority remains a distinct opaque, least-privilege, immutable grant bound to the exact workspace, run, task, attempt generation, ForkPin, context, receipt, and proposal.

Only the pinned, provenance-verified native Claude Code or Codex executable may use its normal host-native authentication store under that OS identity. The adapter receives the Horseness attempt capability but MUST NOT receive bearer tokens, cookies, OAuth material, auth-file contents, environment-exported secrets, or a fabricated credential reference representing the subscription session. Horseness code MUST NOT open, parse, copy, hash, back up, journal, export, print, or persist the native authentication store or secret values; manufacture authentication environment variables; or place subscription state in fixtures, packages, receipts, logs, or artifacts. Restart, reconcile, and resume reuse the bound native host session without copying authentication state into adapter-owned retained state.

This exception explicitly accepts the user's existing provider account scope and subscription billing. It claims no least-privilege provider test account, opaque provider credential reference, monetary cap, or token-cost cap. It does not weaken native package and contribution provenance, host-contribution confinement, immutable Horseness bindings, authority-produced decisions, output/evidence redaction, or byte bounds. Each live scenario freezes exact request fixtures and scenario count, maximum provider operations and turns, allowed tools and network purpose, input/output/evidence byte ceilings, and a finite wall-clock watchdog. These deterministic non-monetary bounds prevent unbounded execution but are not a provider cost guarantee.

The real native host MUST attest through a bounded native status or invocation path that a usable logged-in session exists without exposing secret material. Missing, expired, rejected, wrong-host, model-unavailable, interactive-login-required, timed-out, redaction-failed, or native-host-failed sessions are hard acceptance failures, never skips or fallbacks. Stable redacted reason codes include `LIVE_SUBSCRIPTION_SESSION_ABSENT`, `LIVE_SUBSCRIPTION_SESSION_INVALID`, `LIVE_INTERACTIVE_LOGIN_REQUIRED`, `LIVE_MODEL_UNAVAILABLE`, `LIVE_REDACTION_FAILURE`, `LIVE_TIMEOUT`, and `LIVE_HOST_FAILURE`. Tests MUST NOT attempt interactive login or credential refresh.

Subscription-session live receipts are distinct from hermetic validator receipts and from the other host's live receipt. They record only schema/version, `authMode: existing-user-subscription-session`, host identity, pinned native distribution/version/archive/member/executable digests, contribution digest, observed model identifier, candidate commit/tree, exact command and scenario-set digest, immutable Horseness bindings, output/evidence digests and redaction audit, start/end/duration, terminal result, and stable reason code. They MUST omit account identifiers, emails, subscription IDs, authentication paths, token fingerprints, credential references, request headers, secret-derived hashes, and all secret material.

Adapter uninstall still revokes Horseness grants and disables or removes the native contribution. It MUST NOT claim to revoke or delete the user's Claude Code or Codex subscription session; provider-session revocation remains the responsibility of native host logout or provider account controls.
...
Each required host must ship a loadable native contribution: Pi extension plus skills, OMP plugin/skills metadata, Claude Code plugin with namespaced command/skill/agent/hook contributions, and Codex plugin/skill/MCP contribution. Minimum exercised capability is discovery, load, context injection, one attempt through a local deterministic provider or controlled live account, receipt binding, restart/reconcile or declared unsupported result, and uninstall. For Claude Code and Codex, the subscription-session exception above replaces the controlled-live-account provider-authentication prerequisite while preserving the same native contribution and behavioral proof. CLI-only mode is additional degradation, never a substitute. C11 blocks and requires an explicit scope replan if an official native mechanism cannot meet this minimum.

Hermetic real-binary/official-validator gates are mandatory and never skip. Credentialed live gates generally use pinned host/model/account provenance, opaque CI secret references, rotation/revocation, redaction, and budgets; unavailable required live evidence fails closed at publication. Claude Code and Codex instead use the subscription-session exception above: `authMode: existing-user-subscription-session`, no opaque CI provider secret reference, and no Horseness-managed provider-session rotation or revocation, while provenance, redaction, deterministic bounds, and fail-closed evidence remain mandatory. Every adapter smoke covers daemon restart, host restart, session/thread resume where supported, fork switch, and receipt-to-manifest matching.
...
The first public release trusts npm registry integrity and npm-generated provenance tied to the protected GitHub release workflow. Release packages contain no custom project-root key chain, KMS signature, immutable-storage receipt, or offline archive. The fixture-trust bootstrap remains repository-only and MUST NOT be published as a production entry point. A future self-contained or offline bootstrap requires a new ADR that restores an independently reviewed distribution trust design before public use.
...
### Neutral install bundles and deferred bootstrap

The installer core consumes only `NeutralInstallBundleV1`; it MUST NOT import an adapter package or execute adapter contribution code. Its existing signed-manifest, catalog, contribution-byte, consent, replay, and path-confinement checks remain runtime/test invariants. For the npm-first release, however, no production signed bootstrap envelope is published: `@horseness/bootstrap`, the public project trust root, and offline media are deferred. Repository fixture envelopes exercise the installer contract but are never public release evidence.

Installation separates the scope root from host discovery. For user scope, POSIX state/data roots are `${XDG_STATE_HOME:-$HOME/.local/state}/horseness/install` and `${XDG_DATA_HOME:-$HOME/.local/share}/horseness/install`; Windows roots are `%LOCALAPPDATA%\Horseness\Install\state` and `%LOCALAPPDATA%\Horseness\Install\data`. For workspace scope they are `<realpath-workspace>/.horseness/install/state` and `<realpath-workspace>/.horseness/install/data` on every platform. Verified contribution bytes live only below the selected data root at `bundles/<releaseManifestDigest>/<hostId>/`; journal, refcount, kill-switch, and ownership records live below the selected state root. A signed catalog entry names one closed discovery-root identifier and a confined relative target. The executable edge resolves that identifier to the invoking user's native host home: Pi `${PI_CODING_AGENT_HOME:-$HOME/.pi/agent}` / `%USERPROFILE%\.pi\agent`; OMP `${OMP_HOME:-$HOME/.omp}` / `%USERPROFILE%\.omp`; Claude `${CLAUDE_CONFIG_DIR:-$HOME/.claude}` / `%USERPROFILE%\.claude`; Codex `${CODEX_HOME:-$HOME/.codex}` / `%USERPROFILE%\.codex`. Environment overrides must be absolute, owner-controlled, and realpath-confined; relative, symlinked, cross-owner, or ambiguous roots fail before mutation. Discovery registrations point from the host root to the verified managed bytes; arbitrary catalog paths and writes outside these roots are forbidden.

Every managed target has an owner-private marker under the state root binding schema version, scope/workspace, OS identity, release-manifest and package/source digests, host and contribution identity, resolved target, content digest, mode, shared-content refcount identity, and journal operation. Discovery is activated only after the marker and bytes are durable. Existing unmarked targets are `managed-blocked`; a matching marker/content pair is idempotently owned; marker/content disagreement is drift and never overwritten. POSIX roots are `0700`, records `0600`, executable contributions at most `0700`, and non-executable bytes at most `0600`. Windows uses non-inheriting DACLs limited to the invoking SID and administrators and removes execute permission from quarantine. Unsupported platform/arch or absent pinned native-host support is a per-host `unsupported` result: it performs no mutation, is an error for an explicitly requested host, and contributes to exit `3` in a non-atomic multi-host request or compensation/failure in `--atomic-hosts`; it is never silently treated as absent or success. In particular, a Linux-only Claude or Codex release record is `unsupported` on macOS/Windows until the signed catalog contains a verified tuple for that platform.

`doctor` uses C20 pure read-only inspectors over supplied paths/bytes and public daemon status. Inspectors may open and hash existing bytes and parse journal/state generations without creating a directory or file, chmod/DACL change, lock acquisition, recovery write, migration, process, network, database, keychain, host load, or contribution import. Missing, corrupt, or unknown-newer state is data in the versioned doctor result, not a reason to call a mutating `open()` or repair path.

The npm release train contains exactly fourteen public packages: eight core packages, daemon, CLI, and four adapters. C22 packs the train twice and verifies identical tarballs. C23 publishes the exact version under the `next` dist-tag with npm provenance. C24 installs and exercises those exact public packages on Linux, macOS, and Windows. C25 moves `latest` only after C24 succeeds and then creates the GitHub release. `@horseness/bootstrap`, offline archives, custom signing roots, custom immutable storage, and custom release receipts are deferred.
...
### npm release and promotion state machine

The first release uses npm registry integrity, npm-generated provenance, repository source, and a protected GitHub `release` environment. Initial package creation MAY use a short-lived package-scoped granular npm token because npm trusted publishers can be configured only after a package exists. Subsequent package publication SHOULD use npm trusted publishing from `.github/workflows/release.yml`. npm trusted publishing authorizes publish/stage operations, not dist-tag mutation; C25 therefore uses either maintainer 2FA or a separately provisioned short-lived package-scoped promotion token. No persistent release token is retained. Static signing keys, custom KMS signers, project-root ceremonies, and custom immutable-storage credentials are outside the v1 release path.

Publication state is `candidate_packed -> next_published -> public_verified -> latest_promoted -> github_released`. A retry looks up each exact npm version and accepts it only when registry integrity matches the packed tarball. C24 verifies exact public packages before C25 moves `latest`. GitHub Actions status and npm registry metadata are operational evidence; custom signed journals, artifact receipts, promotion receipts, and release-specific live receipts are not created.
