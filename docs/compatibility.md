...
`@horseness/policy` writes immutable `PolicyDocumentV1` values addressed by the domain-separated `horseness.policy.v1` digest and strict `PolicyReferenceStateV1` lifecycle values. Readers reject unknown versions, kinds, effects, extra fields, malformed lineage, duplicate or noncanonical rule/evidence identifiers, and substituted document digests before evaluation.

Admission is a pure v1 function over explicit authority time and composite observation cursor. It independently evaluates pinned and current documents, treats a null reference as `NoPolicyV1`, and deterministically conjoins every result, constraint, and explanation. Evidence identity binds digest, logical path, and version. Approval compatibility binds both policy digests, proposal/base tuple, action, grant digest, cursor, and `[issuedAt, expiresAt)` authority-time validity; no v1 reader carries an approval across replacement or cursor movement.
