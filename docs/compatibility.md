...
Codex support is pinned to the digest-verified `@openai/codex@0.144.1-linux-x64` archive and executable SHA-256 `a96f944d1a596dbfb7fdd84f482be5c50e34b04bb371126840d873e4ebf26902`. Its plugin manifest, MCP declaration, context contribution, skill, and MCP server are independently hashed in shipped and natively installed cache roots. Codex 0.144.1 does not interpolate a plugin-root token for local stdio MCP paths, so the supported per-thread absolute override is derived only from the enabled native `plugin/installed`/`plugin/read` record and verified declaration bytes. The pinned host's tool item has nullable `pluginId` under that override; provenance records reason `PINNED_HOST_THREAD_OVERRIDE` instead of claiming native item attribution.

The live gate uses ADR 0006's invoking-user subscription session without inspecting or mutating provider authentication state. Each app-server thread starts the verified installed MCP server with a one-time claim, then binds that claim after the native thread response to the actual thread identity, immutable capability/ForkPin, and verified installed context/skill bytes. A real model-native batch originates five complete durable, tuple-bound `WorkerReturnV1` deliveries and surfaces all five authority outcomes. The gate terminates and recreates app-server/runtime/retained state before native resume, reconcile/reattach, and fork. Crash-injected uninstall persists and fsyncs a kill switch, reconciles plugin/marketplace and absolute-override discovery disable before any fresh thread, revokes authority, rejects cached claims, and proves native inventory absence without touching Codex authentication.
