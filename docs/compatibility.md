...
Codex support is pinned to the digest-verified `@openai/codex@0.144.1-linux-x64` archive and executable SHA-256 `a96f944d1a596dbfb7fdd84f482be5c50e34b04bb371126840d873e4ebf26902`. Its plugin manifest, MCP declaration, context contribution, skill, and MCP server are independently hashed. Codex 0.144.1 does not interpolate a plugin-root token for local stdio MCP paths, so the declaration remains contribution-relative adapter/installer input and every native start/resume/fork applies the supported snake-case `config.mcp_servers` override resolved to the exact absolute verified server bytes.

The live gate uses ADR 0006's invoking-user subscription session without inspecting or mutating provider authentication state. It drives pinned app-server JSON-RPC (`initialize`/`initialized`, native marketplace/plugin lifecycle, thread start/resume/fork, MCP readiness/inventory, and model turn events), explicitly supplies the installed skill as `UserInput`, and accepts only a real `mcpToolCall` attributed to server `horseness-worker`, tool `horseness_worker_return`, and its plugin provenance. One model-native batch originates five complete durable, tuple-bound `WorkerReturnV1` deliveries and surfaces accepted, rejected, conflicted, quarantined, and approval-required authority outcomes. Acceptance additionally requires canonical revision advance, immutable thread resume/fork mapping, exact ForkPin/context binding, candidate-bound redacted receipt evidence, and crash-idempotent kill-switch/native-discovery-disable/Horseness-revocation uninstall ordering with fresh-thread inventory absence. Direct `mcpServer/tool/call`, Claude CLI/parser conventions, and CLI-only behavior cannot satisfy this row; uninstall never logs out the provider session.
