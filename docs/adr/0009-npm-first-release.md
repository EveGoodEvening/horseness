...
- The first publication uses a short-lived, package-scoped npm granular token held only in the protected GitHub environment because trusted publisher configuration requires each package to exist. Subsequent package publication should configure npm trusted publishing for `release.yml`; npm trusted publishing requires npm 11.5.1 or newer and Node 22.14.0 or newer. Because trusted publishing does not authorize dist-tag mutation, C25 uses either maintainer 2FA or a separately provisioned short-lived package-scoped promotion token. No persistent npm release token is retained.
