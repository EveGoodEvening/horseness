...
C22 is deliberately fail-closed. The offline root ceremony is performed and independently reviewed before any delegated signing, upload, or publication effect. `release:verify-root-ceremony` validates the exact `RootCeremonyRecordV1` shape, distinct Ed25519 root and recovery keys, SPKI fingerprints, 2-of-2 root threshold, 2-of-3 recovery custody, witnesses, and content-addressed custody/destruction evidence. The repository contains the schema but does not contain a fabricated ceremony record or evidence.
