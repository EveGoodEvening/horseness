...
Prospective C22 reconciliation is explicit: any release/live-gates implementation owned by C22 MUST honor the adapter-owned Claude and Codex receipt contract with `authMode: existing-user-subscription-session`. It MUST NOT apply the legacy absent-credential-reference skip, require an opaque CI provider secret reference, or require provider-session rotation/revocation for those two hosts. C22 may consume the adapter-owned receipts and schemas without rewriting C11 history. If implementation proves that `.github/workflows/live-gates.yml` or another shared live matrix must change to express this exception, C22 MUST first receive a narrow ordinary reviewed ownership correction naming the exact shared paths and supplemental gates; no shared-matrix edit is authorized by this correction alone.