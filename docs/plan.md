...
## Execution, ownership, claim, and evidence protocol
...
2. Public/persisted/capability/install-format changes also own their fixtures, `docs/compatibility.md`, `docs/migrations.md` where applicable, and one new `.changeset/*.md`. Architectural changes require a planning correction and ADR before implementation resumes.
3. Before source edits, the authorized serialized integrator performs an atomic claim CAS on the integration branch against expected dependency receipt IDs and status: update `docs/progress.md` and the chunk ledger from `docs/progress/template.md`, commit `chore(progress): claim <ID>`, integrate it, then publish its SHA as the claim receipt in `docs/checkpoints/index.jsonl`. Competing/stale worktrees are rejected. Renewal recomputes expiry and commits `chore(progress): renew <ID>`; release commits `chore(progress): release <ID>`. The integrated summary is authoritative if a branch ledger diverges.
4. Implementation produces an immutable candidate commit/tree. Local and remote acceptance run against that SHA. The serialized integrator appends `docs/checkpoints/<ID>.json` and one checksummed line to `docs/checkpoints/index.jsonl` containing candidate SHA/tree, dependency receipt IDs, commands/results, CI run URLs/IDs, signer, timestamp, and final integration SHA, then commits `chore(progress): attest <ID>`. The chunk ledger stores the receipt path/digest, not its own hash. Dependencies consume only an integrated valid receipt.
5. Commands introduced by a chunk run only after it creates them. Every changed TypeScript package is explicitly typechecked. Adapter/executable-edge chunks run `boundaries:check`. Source defects block the detecting chunk; validation chunks never edit unowned source.

In every chunk entry, “manifest/lock/changeset/ledger/integration” expands exactly to the affected package's listed `package.json`, repository `pnpm-lock.yaml`, the named `.changeset/<ID>-*.md`, `docs/progress/<ID>.md`, `docs/progress.md`, `docs/checkpoints/<ID>.json`, and `docs/checkpoints/index.jsonl`. “Ledger/integration artifacts” expands to the last four paths only. “Compatibility/feasibility docs” means `docs/compatibility.md` and `docs/integrations/feasibility.md`. No shorthand grants ownership outside these expansions. Before C00 creates receipt files, its claim is the planning baseline exception recorded in the C00 ledger.

## Remediation and revalidation protocol

Durable findings are `FNNN` receipts under `docs/findings/`. A planning correction checkpoint may append stable remediation IDs `RNNN` and revalidation IDs `VNNN`; IDs are never renumbered. `RNNN` depends on the finding receipt and last-good source receipts, not the blocked detector. `VNNN` depends on the remediation and reruns a stale chunk's full acceptance. Claimable statuses are only `not-started` or `stale` with all dependencies satisfied; `blocked`, `claimed`, and unremediated `stale` are ineligible. Resume prioritizes dependency-ready `R`, then `V`, then base `C` IDs. Planning correction atomically updates this plan, progress count/index, new ledgers, dependencies, and checkpoint subjects.

Example: C21 finds a C07 defect -> append `F001`; keep C21 blocked; add R001 depending on F001+C06 and owning the precise C07 source/tests; mark C07–C20 evidence stale; add V001…V014 chained from R001 to rerun C07–C20; then resume C21. A C22/C24 finding works identically and is not blocked by numeric order. An early-chunk finding marks only affected descendants stale.
