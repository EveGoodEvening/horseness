...
`publishReconstructedContext(authority, reconstructed)` is the C09 publication boundary. It canonical-encodes the complete `ContextManifestRecordV1` as UTF-8 `manifestBytes` and constructs an immutable `ContextManifestPublicationV1` containing workspace/run/attempt/generation plus `contextManifestCoreDigest`, `attemptContextBindingDigest`, and `renderedOutputDigest`. It then delegates to `ContextManifestPublicationAuthorityV1.publishDurablyReferencedManifest`. That authority owns the durable publish-and-reference transaction above; C09 does not report success if it throws or is interrupted.

Only a reconstruction object issued by `reconstructPinnedContext` can cross this boundary. Issued results are recorded in a private runtime membership set and their manifest/binding records are deeply frozen; a structurally identical caller-created object is not authority. The SQLite publication request also carries the complete `AttemptContextBindingV1` preimage. Before publishing bytes or appending an event, the store independently recomputes both domain-separated digests and requires exact manifest/binding equivalence for attempt, generation, ForkPin, source cursor/version, authorization cursor/version, and manifest digest.

An event or database reference must never commit before the complete artifact is published. A crash before rename may leave staging bytes, which recovery removes. A crash after rename but before the SQLite commit may leave an unreferenced complete object, which is safe and may later be reused or collected. A crash after the transaction leaves both the immutable object and authoritative reference. Retrying publication from the same reconstruction passes byte-identical canonical manifest bytes and the same three digests to the authority. Digest collision/substitution, mismatched size or media metadata, a missing referenced object, or corrupt bytes fails closed; reconstruction, dispatch, admission, and replay must not substitute or regenerate trusted bytes from metadata.
