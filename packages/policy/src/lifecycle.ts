import { DomainError, domainDigest, type JsonValue } from "@horseness/domain";

export type PolicyEffectV1 = "accepted" | "rejected" | "quarantined" | "approval_required";
export interface PolicySubjectV1 {
  action: string | null;
...
export interface PolicyDocumentCoreV1 {
  schemaVersion: "1";
  kind: "policy";
  policyId: string;
  revision: number;
...
export interface PolicyDocumentV1 {
  core: PolicyDocumentCoreV1;
  policyDigest: string;
}
export type PolicyReferenceStateV1 =
  | { schemaVersion: "1"; state: "inactive"; activePolicyDigest: null; activationSequence: number }
...
function sortedUnique(values: unknown, code: string): asserts values is string[] {
  ...
  if (values.some((value, index) => index > 0 && values[index - 1]! >= value)) fail(code);
}
function validateSubject(value: unknown): void {
  const item = record(value, ["action", "pathPrefix", "version"], "POLICY_SUBJECT_INVALID");
  nullableText(item.action, "POLICY_SUBJECT_INVALID"); nullableText(item.pathPrefix, "POLICY_SUBJECT_INVALID"); nullableText(item.version, "POLICY_SUBJECT_INVALID");
  if (typeof item.pathPrefix === "string" && !item.pathPrefix.startsWith("/")) fail("POLICY_SUBJECT_INVALID");
}
function validateEvidence(value: unknown): void {
  ...
  text(item.evidenceId, "POLICY_EVIDENCE_INVALID"); text(item.digest, "POLICY_EVIDENCE_INVALID"); text(item.path, "POLICY_EVIDENCE_INVALID"); text(item.version, "POLICY_EVIDENCE_INVALID");
  if (!(item.path as string).startsWith("/")) fail("POLICY_EVIDENCE_INVALID");
}
function validateRule(value: unknown): void {
  ...
  if (!Array.isArray(item.evidence)) fail("POLICY_RULE_INVALID");
  item.evidence.forEach(validateEvidence);
  const ids = item.evidence.map((entry) => (entry as EvidenceRequirementV1).evidenceId);
  if (new Set(ids).size !== ids.length || ids.some((id, index) => index > 0 && ids[index - 1]! >= id)) fail("POLICY_RULE_INVALID");
}
export function parsePolicyDocumentCoreV1(value: unknown): PolicyDocumentCoreV1 {
  ...
  if (core.schemaVersion !== "1") fail("POLICY_VERSION_UNSUPPORTED");
  if (core.kind !== "policy") fail("POLICY_KIND_UNSUPPORTED");
  ...
  if (!Array.isArray(core.rules)) fail("POLICY_DOCUMENT_INVALID");
  core.rules.forEach(validateRule);
  const ruleIds = core.rules.map((rule) => (rule as PolicyRuleV1).ruleId);
  if (new Set(ruleIds).size !== ruleIds.length || ruleIds.some((id, index) => index > 0 && ruleIds[index - 1]! >= id)) fail("POLICY_RULE_ORDER_INVALID");
  return value as PolicyDocumentCoreV1;
}
export function policyDocumentDigest(core: PolicyDocumentCoreV1): string {
  parsePolicyDocumentCoreV1(core);
  return domainDigest("horseness.policy.v1", core as unknown as JsonValue);
}
export function sealPolicyDocument(core: PolicyDocumentCoreV1): PolicyDocumentV1 {
  return Object.freeze({ core: structuredClone(parsePolicyDocumentCoreV1(core)), policyDigest: policyDocumentDigest(core) });
}
export function parsePolicyDocumentV1(value: unknown): PolicyDocumentV1 {
  const document = record(value, ["core", "policyDigest"], "POLICY_DOCUMENT_INVALID");
  const core = parsePolicyDocumentCoreV1(document.core); text(document.policyDigest, "POLICY_DOCUMENT_INVALID");
  if (document.policyDigest !== policyDocumentDigest(core)) fail("POLICY_DIGEST_MISMATCH");
  return value as PolicyDocumentV1;
}
export function activatePolicy(state: PolicyReferenceStateV1, document: PolicyDocumentV1): PolicyReferenceStateV1 {
  parsePolicyDocumentV1(document); parsePolicyReferenceStateV1(state);
  if (state.activePolicyDigest === document.policyDigest) return state;
...
export function deactivatePolicy(state: PolicyReferenceStateV1): PolicyReferenceStateV1 {
  parsePolicyReferenceStateV1(state);
  if (state.state === "inactive") return state;
  return Object.freeze({ schemaVersion: "1", state: "inactive", activePolicyDigest: null, activationSequence: state.activationSequence + 1 });
}
export function parsePolicyReferenceStateV1(value: unknown): PolicyReferenceStateV1 {
  ...
  if (state.schemaVersion !== "1") fail("POLICY_VERSION_UNSUPPORTED"); natural(state.activationSequence, "POLICY_REFERENCE_INVALID");
  if (state.state === "active") text(state.activePolicyDigest, "POLICY_REFERENCE_INVALID");
  else if (state.state === "inactive") { if (state.activePolicyDigest !== null) fail("POLICY_REFERENCE_INVALID"); }
  else fail("POLICY_REFERENCE_INVALID");
  return value as PolicyReferenceStateV1;
}
