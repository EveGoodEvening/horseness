...
import {
  ...
  type WorkspaceOperationalEvent,
  type WorkspaceState,
...
export function verifyAuthority(db:DatabaseSync, artifactRoot:string, options:{allowPendingRetentionMissing?:boolean}={}):{streams:number;events:number;artifacts:number}{
  ...
  for(const stream of streams){
    ...
    const last=chain.at(-1);if(rows.length!==stream.head_sequence||(last?.envelopeHash??null)!==stream.head_hash)throw new RecoveryIntegrityError("stream head mismatch");
    try {
      if (stream.stream_kind === "run") {
        deterministicReplay(chain);
      } else if (stream.stream_kind === "workspace") {
        let state: WorkspaceState | null = null;
        for (const item of chain) {
          const payload = parseDomainEventPayloadV1(item.envelope.payload);
          if (payload.eventType !== "WorkspaceCreatedV1" && payload.eventType !== "PolicyReferenceChangedV1") throw new Error("workspace stream contains run event");
          const event: WorkspaceOperationalEvent = { ...payload, sequence: item.envelope.sequence };
          state = reduceWorkspaceState(state, event);
        }
      } else {
        throw new Error(`unsupported stream kind ${stream.stream_kind}`);
      }
    } catch (error) {
