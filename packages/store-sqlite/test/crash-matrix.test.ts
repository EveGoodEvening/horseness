...
function request(workspaceId="ws",commandId="atomic-evidence",artifactBytes:Uint8Array=bytes){const genesis=createWorkspaceGenesis({workspaceId,authorityPrincipalId:"authority",initialGrantDigest:"grant",authorityConsumptionMarker:"marker",activePolicyDigest:NO_POLICY_DIGEST,commandId});const artifactDigest=createHash("sha256").update(artifactBytes).digest("hex");return{commandId,workspace:{streamKind:"workspace" as const,workspaceId,streamId:workspaceId,expectedSequence:0,expectedEnvelopeHash:null,events:[genesis.event]},artifacts:[{data:artifactBytes,mediaType:"application/octet-stream",references:[{ownerKind:"event",ownerId:genesis.event.envelope.eventId}],pins:[{pinId:`canonical:${workspaceId}`}]}],requiredArtifactDigests:[artifactDigest],projections:[{workspaceId,name:"workspace",version:"1",streamKind:"workspace" as const,streamId:workspaceId,lastSequence:1,lastEnvelopeHash:genesis.event.envelopeHash}]};}
...
test("required artifact without a workspace event reference rolls back",()=>{const root=mkdtempSync(join(tmpdir(),"horseness-unbound-"));try{const store=new SQLiteAuthority(join(root,"db.sqlite"),join(root,"artifacts"));const valid=request();const invalid={...valid,artifacts:valid.artifacts.map(publication=>({data:publication.data,mediaType:publication.mediaType,pins:publication.pins}))};assert.throws(()=>store.publishAndAppendAtomic(invalid),/required artifact has no workspace event reference/);assert.equal((store.db.prepare("SELECT count(*) AS count FROM events").get() as {count:number}).count,0);assert.equal((store.db.prepare("SELECT count(*) AS count FROM artifact_refs").get() as {count:number}).count,0);store.close();}finally{rmSync(root,{recursive:true,force:true});}});

test("nonexistent and cross-workspace event owners are rejected",()=>{const root=mkdtempSync(join(tmpdir(),"horseness-owner-"));try{const store=new SQLiteAuthority(join(root,"db.sqlite"),join(root,"artifacts"));const first=request("workspace-a","shared-event");store.publishAndAppendAtomic(first);const ownerId=first.workspace.events[0]!.envelope.eventId;for(const [commandId,owner] of [["missing-owner","does-not-exist"],["cross-owner",ownerId]] as const){const base=request("workspace-b",commandId,Buffer.from(commandId));const candidate={...base,artifacts:base.artifacts.map(publication=>({...publication,references:[{ownerKind:"event",ownerId:owner,allowExistingEvent:true as const}]}))};assert.throws(()=>store.publishAndAppendAtomic(candidate),/does not exist in workspace/);assert.equal((store.db.prepare("SELECT count(*) AS count FROM events WHERE workspace_id='workspace-b'").get() as {count:number}).count,0);}store.close();}finally{rmSync(root,{recursive:true,force:true});}});

test("same event ids and reference removal are isolated by workspace",()=>{const root=mkdtempSync(join(tmpdir(),"horseness-ref-scope-"));try{const store=new SQLiteAuthority(join(root,"db.sqlite"),join(root,"artifacts"));const first=request("workspace-a","shared-event");const second=request("workspace-b","shared-event");store.publishAndAppendAtomic(first);store.publishAndAppendAtomic(second);const ownerId=first.workspace.events[0]!.envelope.eventId;assert.equal(second.workspace.events[0]!.envelope.eventId,ownerId);assert.equal((store.db.prepare("SELECT count(*) AS count FROM artifact_refs WHERE owner_id=? AND digest=?").get(ownerId,digest) as {count:number}).count,2);store.artifacts.removeReference("workspace-a","event",ownerId,digest);const remaining=store.db.prepare("SELECT workspace_id FROM artifact_refs WHERE owner_id=? AND digest=? ORDER BY workspace_id").all(ownerId,digest) as unknown[];assert.equal(remaining.length,1);const row=remaining[0];assert.ok(row!==null&&typeof row==="object"&&"workspace_id" in row&&typeof row.workspace_id==="string","expected SQLite row with a string workspace_id");assert.equal(row.workspace_id,"workspace-b");store.close();}finally{rmSync(root,{recursive:true,force:true});}});
