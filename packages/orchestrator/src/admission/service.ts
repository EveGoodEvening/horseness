...
function loadAuthorities(authority:SQLiteAuthority, request:AdmissionRequestV1, observed:CompositeCursorV1):{historical:AdmissionSealingAuthorityV1;current:AdmissionCurrentAuthorityV1} {
  ...
  const currentValue=authorityRecord(currentAuthorityAt(authority,core.workspaceId,core.runId,observed),"AUTHORITY_STATE_SUBSTITUTED");
...
function validateRequest(request: AdmissionRequestV1, revision: RevisionView, historical:AdmissionSealingAuthorityV1, current:AdmissionCurrentAuthorityV1): { delta: DeltaResult; paths: string[]; snapshots:SnapshotExpectationV1; evaluationClock:EvaluationClockV1 } {
  ...
  const requestedPolicyDigest=request.action==="release"?policySlotDigest(current.currentPolicy):core.currentPolicyDigest;
  if(request.scopeDigest!==core.deltaAuthorityScopeDigest||request.forkPinDigest!==core.forkPinDigest||!same([...request.receiptDigests].sort(),[...core.receiptDigests].sort())||request.policyDigest!==requestedPolicyDigest||request.quotaId!==current.quota.id||!same([...request.evidenceIds].sort(),historical.evidence.map(item=>item.evidenceId).sort()))fail("AUTHORITY_IDENTIFIER_MISMATCH");
...
export class AdmissionService {
  ...
  evaluateAndApply(request: AdmissionRequestV1): AdmissionResultV1 {
    ...
    if(delta.outcome!=="conflicted"&&request.action!=="release"&&policySlotDigest(loaded.current.currentPolicy)!==core.currentPolicyDigest)fail("POLICY_SUBSTITUTED");
