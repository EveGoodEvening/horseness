...
import { assertImmutableBinding, isBoundAttempt, type BoundAttemptV1 } from "../attempts/attempts.js";
import { assertDispatchBinding, handoffCancellation, isDispatchRecord, lookupDispatch, recordProviderHandle, requestDispatchCancellation, type DispatchRecordV1, type ProviderAdapterV1 } from "../dispatch/dispatch.js";
...
export function recoverDispatch(input:{record:DispatchRecordV1;attempt:BoundAttemptV1;lease:LeaseFenceV1;ownerId:string;fenceToken:number;now:string;adapter:ProviderAdapterV1}):RecoveryResultV1{
 ...
 try{record=lookupDispatch(record,input.adapter);}catch{return result(record,input.lease,"unknown","UNKNOWN_OUTCOME",true);}
