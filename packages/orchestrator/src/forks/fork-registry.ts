...
function authenticate(context: ForkAuthorityContextV1): ForkAuthoritySnapshotIdentitiesV1 {
  ...
  if (!source.authenticated || !authorization.authenticated) fail("FORK_AUTHORITY_UNAUTHENTICATED");
  try {
...
export function createProjectedFork(state: ForkProjectionV1, forkId: string, context: ForkAuthorityContextV1): { state: ForkProjectionV1; pin: SealedForkPinV1 } {
  const identities = authenticate(context);
...
export function refreshProjectedFork(state: ForkProjectionV1, forkId: string, context: ForkAuthorityContextV1): { state: ForkProjectionV1; pin: SealedForkPinV1 } {
  const identities = authenticate(context);
...
export function reuseProjectedFork(state: ForkProjectionV1, forkId: string, context: ForkAuthorityContextV1): SealedForkPinV1 {
  const identities = authenticate(context);
  const digest = state.activeByForkId.get(forkId);
