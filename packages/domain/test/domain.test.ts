...
test("proposal validators reject malformed, future and substituted wire values", () => {
  const core: ProposalEnvelopeCoreV1 = { schemaVersion: "1", workspaceId: "ws", runId: "run", authorPrincipalId: "worker", authorGrantDigest: "grant", attemptId: "attempt", receiptDigests: ["receipt"], forkPinDigest: "pin", deltaAuthorityScopeDigest: deltaAuthorityScopeDigest(scope), baseRevision: 0, baseStateHash: "state", canonicalizerVersion: "jcs-v1", hashVersion: "sha256-v1", proposalSealingObservationCursor: composite, proposalSealingContextVersion: { schemaVersion: "1", kind: "composite", workspaceContextEpoch: 0, runContextEpoch: 0, observationCursor: composite }, operations: [{ op: "test", path: "/items", expectedValueDigest: jsonValueDigest([]) }], evidenceClaims: [{ digest: "evidence", claim: "supports-change" }], pinnedPolicyDigest: NO_POLICY_DIGEST, currentPolicyDigest: NO_POLICY_DIGEST, nonce: "nonce", predecessorProposalDigest: null, predecessorReason: null };
  assert.throws(() => sealProposal({ ...core, schemaVersion: "2" } as never), /UNSUPPORTED_SCHEMA_VERSION/);
  assert.throws(() => sealProposal({ ...core, extra: true } as never), /INVALID_ENVELOPE/);
  assert.throws(() => sealProposal({ ...core, operations: [{ op: "future", path: "/items" }] } as never), /INVALID_ENVELOPE/);
  assert.throws(() => sealProposal({ ...core, receiptDigests: ["receipt", "receipt"] }), /INVALID_ENVELOPE/);
  assert.throws(() => sealProposal({ ...core, evidenceClaims: [core.evidenceClaims[0]!, core.evidenceClaims[0]!] }), /INVALID_ENVELOPE/);
  assert.throws(() => sealProposal({ ...core, proposalSealingContextVersion: { schemaVersion: "1", kind: "composite", workspaceContextEpoch: 0, runContextEpoch: 1, observationCursor: composite } }), /INVALID_ENVELOPE/);
  const sealed = sealProposal(core); assert.throws(() => verifyProposal({ ...sealed, unknown: true } as never), /INVALID_ENVELOPE/);
});
...
test("receipt validators reject malformed, future, duplicate and outcome-invalid values", () => {
  const core = { schemaVersion: "1", workspaceId: "ws", runId: "run", taskId: "t", attemptId: "a", generation: 1, attemptContextBindingDigest: "b", contextManifestCoreDigest: "m", forkPinDigest: "f", providerId: "provider", providerOperationId: "operation", providerIdempotencyKeyDigest: "k", producerPrincipalId: "adapter", producerGrantDigest: "g", adapterId: "adapter", adapterVersion: "1", hostId: "host", hostVersion: "1", outcome: "succeeded", startedAt: "2026-01-01T00:00:00Z", finishedAt: "2026-01-01T00:00:01Z", outputDigest: "output", evidence: [{ digest: "e", mediaType: "text/plain", size: 1 }], provenance: {}, nonce: "n" } as const;
  assert.throws(() => sealAttemptReceipt({ ...core, schemaVersion: "2" } as never), /UNSUPPORTED_SCHEMA_VERSION/);
  assert.throws(() => sealAttemptReceipt({ ...core, extra: true } as never), /INVALID_ENVELOPE/);
  assert.throws(() => sealAttemptReceipt({ ...core, outcome: "future" } as never), /INVALID_ENVELOPE/);
  assert.throws(() => sealAttemptReceipt({ ...core, startedAt: "2026-01-01T00:00:00+00:00" }), /INVALID_ENVELOPE/);
  assert.throws(() => sealAttemptReceipt({ ...core, evidence: [core.evidence[0], core.evidence[0]] }), /INVALID_ENVELOPE/);
  assert.throws(() => sealAttemptReceipt({ ...core, outcome: "failed", outputDigest: "output" } as never), /INVALID_ENVELOPE/);
  const receipt = sealAttemptReceipt(core); assert.throws(() => verifyAttemptReceipt({ ...receipt, unknown: true } as never), /INVALID_ENVELOPE/);
});
test("canonical reducer enforces identity, monotonicity and immutable documents", () => {
  ...
  const final = { nested: { n: 1 } }; const accepted = reduceCanonicalDocument(genesis, { eventType: "DeltaAcceptedV1", sequence: 2, workspaceId: "ws", runId: "run", proposalId: "p", resultingDocument: final, priorStateHash: genesis.stateHash, resultingStateHash: domainDigest("horseness.canonical-document.v1", final) });
  ...
  assert.throws(() => reduceCanonicalDocument(accepted, { eventType: "DeltaAcceptedV1", sequence: 2, workspaceId: "ws", runId: "run", proposalId: "p2", resultingDocument: final, priorStateHash: accepted.stateHash, resultingStateHash: accepted.stateHash }), /EVENT_SEQUENCE_INVALID/);
...
test("workspace and run operational reducers are exhaustive, monotonic and immutable", () => {
  ...
  const admitted = reduceWorkspaceState(changed, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 3, workspaceId: "ws", proposalDigest: "proposal-a", decisionEventId: "decision-a", state: "accepted", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" });
  const rejected = reduceWorkspaceState(admitted, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 4, workspaceId: "ws", proposalDigest: "proposal-b", decisionEventId: "decision-b", state: "rejected", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" });
  assert.deepEqual(admitted.admissions["proposal-a"], { proposalDigest: "proposal-a", decisionEventId: "decision-a", state: "accepted", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes", history: [{ decisionEventId: "decision-a", state: "accepted", consumed: "yes" }] });
  assert.deepEqual(rejected.quotas.quota, { quotaDigest: "quota-v1", consumedDecisionEventIds: ["decision-a"], observedDecisionEventIds: ["decision-a", "decision-b"] });
  assert.equal(rejected.activePolicyDigest, "policy-2"); assert.deepEqual(changed.admissions, {}); assert.deepEqual(changed.quotas, {});
  assert.throws(() => reduceWorkspaceState(rejected, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 5, workspaceId: "ws", proposalDigest: "proposal-a", decisionEventId: "decision-c", state: "rejected", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" }), /ILLEGAL_ADMISSION_TRANSITION/);
  assert.throws(() => reduceWorkspaceState(rejected, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 5, workspaceId: "ws", proposalDigest: "proposal-c", decisionEventId: "decision-a", state: "accepted", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" }), /DUPLICATE_ADMISSION_TRANSITION/);
  assert.throws(() => reduceWorkspaceState(rejected, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 5, workspaceId: "ws", proposalDigest: "proposal-c", decisionEventId: "decision-c", state: "accepted", quotaId: "quota", quotaDigest: "quota-v2", consumed: "yes" }), /QUOTA_IDENTITY_CONFLICT/);
  ...
  assert.throws(() => reduceWorkspaceState(changed, { eventType: "PolicyReferenceChangedV1", sequence: 4, workspaceId: "ws", activePolicyDigest: "policy-gap" }), /EVENT_SEQUENCE_INVALID/);
  assert.throws(() => reduceOperationalState(submitted, { eventType: "ForkCreatedV1", sequence: 4, workspaceId: "ws", runId: "run" }), /EVENT_SEQUENCE_INVALID/);
});
test("authenticated workspace replay reconstructs admission and quota projections without policy mutation", () => {
  const payloads = [
    { eventType: "WorkspaceCreatedV1", workspaceId: "ws", authorityPrincipalId: "authority", initialGrantDigest: "grant", authorityConsumptionMarker: "used", activePolicyDigest: "policy" },
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "pending", decisionEventId: "decision-pending", state: "approval_required", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" },
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "pending", decisionEventId: "decision-accepted", state: "accepted", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" },
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "quarantined", decisionEventId: "decision-quarantined", state: "quarantined", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" },
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "quarantined", decisionEventId: "decision-released-rejected", state: "rejected", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" },
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "rejected", decisionEventId: "decision-rejected", state: "rejected", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" }
  ] as const satisfies readonly WorkspaceEventPayloadV1[];
  const chain: HashedEventEnvelopeV1<unknown>[] = []; let priorEnvelopeHash: string | null = null;
  for (let index=0; index<payloads.length; index+=1) { const payload=payloads[index]!; const item: HashedEventEnvelopeV1<WorkspaceEventPayloadV1> = sealEventEnvelope<WorkspaceEventPayloadV1>({schemaVersion:"1",streamKind:"workspace",workspaceId:"ws",streamId:"ws",sequence:index+1,eventId:`w${index+1}`,eventType:payload.eventType,principalId:"authority",causationId:"c",correlationId:"c",idempotencyKey:`w${index+1}`,priorEnvelopeHash,payload}); chain.push(item); priorEnvelopeHash=item.envelopeHash; }
  const replay=deterministicWorkspaceReplay(chain); assert.equal(replay.activePolicyDigest,"policy"); assert.equal(replay.admissions.pending?.state,"accepted"); assert.equal(replay.admissions.pending?.history.length,2); assert.equal(replay.admissions.quarantined?.state,"rejected"); assert.equal(replay.admissions.quarantined?.history.length,2); assert.deepEqual(replay.quotas.quota?.consumedDecisionEventIds,["decision-accepted"]);
  const restarted=deterministicWorkspaceReplay(chain.slice(0,2)); assert.equal(restarted.admissions.pending?.state,"approval_required"); assert.deepEqual(deterministicWorkspaceReplay(chain),replay);
  const append=(payload:Record<string,unknown>,eventId:string)=>sealEventEnvelope({schemaVersion:"1",streamKind:"workspace",workspaceId:"ws",streamId:"ws",sequence:chain.length+1,eventId,eventType:"WorkspaceAdmissionRecordedV1",principalId:"authority",causationId:"c",correlationId:"c",idempotencyKey:eventId,priorEnvelopeHash,payload});
  const duplicateDecision=append({eventType:"WorkspaceAdmissionRecordedV1",workspaceId:"ws",proposalDigest:"new",decisionEventId:"decision-accepted",state:"rejected",quotaId:"quota",quotaDigest:"quota-v1",consumed:"no"},"duplicate-decision"); assert.throws(()=>deterministicWorkspaceReplay([...chain,duplicateDecision]),/DUPLICATE_ADMISSION_TRANSITION/);
  const terminalMutation=append({eventType:"WorkspaceAdmissionRecordedV1",workspaceId:"ws",proposalDigest:"pending",decisionEventId:"decision-third",state:"rejected",quotaId:"quota",quotaDigest:"quota-v1",consumed:"no"},"terminal-mutation"); assert.throws(()=>deterministicWorkspaceReplay([...chain,terminalMutation]),/ILLEGAL_ADMISSION_TRANSITION/);
  const quotaMismatch=append({eventType:"WorkspaceAdmissionRecordedV1",workspaceId:"ws",proposalDigest:"new",decisionEventId:"decision-new",state:"accepted",quotaId:"quota",quotaDigest:"quota-v2",consumed:"yes"},"quota-mismatch"); assert.throws(()=>deterministicWorkspaceReplay([...chain,quotaMismatch]),/QUOTA_IDENTITY_CONFLICT/);
  const consumptionMismatch=append({eventType:"WorkspaceAdmissionRecordedV1",workspaceId:"ws",proposalDigest:"new",decisionEventId:"decision-new",state:"rejected",quotaId:"quota",quotaDigest:"quota-v1",consumed:"yes"},"consumption-mismatch"); assert.throws(()=>deterministicWorkspaceReplay([...chain,consumptionMismatch]),/ADMISSION_QUOTA_INCONSISTENT/);
});
