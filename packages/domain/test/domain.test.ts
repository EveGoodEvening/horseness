...
test("workspace and run operational reducers are exhaustive, monotonic and immutable", () => {
  ...
  const admitted = reduceWorkspaceState(changed, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 3, workspaceId: "ws", proposalDigest: "proposal-a", decisionEventId: "decision-a", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" });
  const rejected = reduceWorkspaceState(admitted, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 4, workspaceId: "ws", proposalDigest: "proposal-b", decisionEventId: "decision-b", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" });
  assert.deepEqual(admitted.admissions["proposal-a"], { proposalDigest: "proposal-a", decisionEventId: "decision-a", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" });
  ...
  assert.throws(() => reduceWorkspaceState(rejected, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 5, workspaceId: "ws", proposalDigest: "proposal-a", decisionEventId: "decision-c", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" }), /DUPLICATE_ADMISSION_TRANSITION/);
  assert.throws(() => reduceWorkspaceState(rejected, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 5, workspaceId: "ws", proposalDigest: "proposal-c", decisionEventId: "decision-a", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" }), /DUPLICATE_ADMISSION_TRANSITION/);
  assert.throws(() => reduceWorkspaceState(rejected, { eventType: "WorkspaceAdmissionRecordedV1", sequence: 5, workspaceId: "ws", proposalDigest: "proposal-c", decisionEventId: "decision-c", quotaId: "quota", quotaDigest: "quota-v2", consumed: "yes" }), /QUOTA_IDENTITY_CONFLICT/);
...
test("authenticated workspace replay reconstructs admission and quota projections without policy mutation", () => {
  const payloads = [
    ...
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "accepted", decisionEventId: "decision-accepted", quotaId: "quota", quotaDigest: "quota-v1", consumed: "yes" },
    { eventType: "WorkspaceAdmissionRecordedV1", workspaceId: "ws", proposalDigest: "rejected", decisionEventId: "decision-rejected", quotaId: "quota", quotaDigest: "quota-v1", consumed: "no" }
  ] satisfies readonly WorkspaceEventPayloadV1[];
  const chain: HashedEventEnvelopeV1<unknown>[] = []; let priorEnvelopeHash: string | null = null;
  for (let index=0; index<payloads.length; index+=1) { const payload=payloads[index]!; const item: HashedEventEnvelopeV1<WorkspaceEventPayloadV1> = sealEventEnvelope({schemaVersion:"1",streamKind:"workspace",workspaceId:"ws",streamId:"ws",sequence:index+1,eventId:`w${index+1}`,eventType:payload.eventType,principalId:"authority",causationId:"c",correlationId:"c",idempotencyKey:`w${index+1}`,priorEnvelopeHash,payload}); chain.push(item); priorEnvelopeHash=item.envelopeHash; }
  const replay=deterministicWorkspaceReplay(chain); assert.equal(replay.activePolicyDigest,"policy"); assert.equal(replay.admissions.accepted?.consumed,"yes"); assert.equal(replay.admissions.rejected?.consumed,"no"); assert.deepEqual(replay.quotas.quota?.consumedDecisionEventIds,["decision-accepted"]);
  const duplicatePayload={eventType:"WorkspaceAdmissionRecordedV1",workspaceId:"ws",proposalDigest:"accepted",decisionEventId:"other",quotaId:"quota",quotaDigest:"quota-v1",consumed:"yes"}; const duplicate=sealEventEnvelope({schemaVersion:"1",streamKind:"workspace",workspaceId:"ws",streamId:"ws",sequence:4,eventId:"w4",eventType:duplicatePayload.eventType,principalId:"authority",causationId:"c",correlationId:"c",idempotencyKey:"w4",priorEnvelopeHash,payload:duplicatePayload}); assert.throws(()=>deterministicWorkspaceReplay([...chain,duplicate]),/DUPLICATE_ADMISSION_TRANSITION/);
});
