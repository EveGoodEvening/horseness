...
test("uninstall routes through installer runtime and preserves exit semantics", async () => { const registry = new CliCommandRegistryV1(); registerInstallerCommandsV1(registry); let called = ""; const exit = await runCliV1(["uninstall", "--workspace", "/tmp/workspace", "--json"], { registry, transport: { request: async () => { throw new Error("unused"); } }, credential: { schemaVersion: "1", kind: "host-reference", reference: "grant:test", scope: { workspaceId: "workspace:test", adapterId: "horseness-cli", purpose: "coordinator" } }, installer: { execute: async (command) => { called = command; return { exitCode: 0, data: { command } }; } }, stdout: () => {}, stderr: () => {} }); assert.equal(called, "uninstall"); assert.equal(exit, 0); });
